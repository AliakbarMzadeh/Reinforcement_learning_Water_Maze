...
eta_values = linspace(0.05, 1, 10);  % Learning rates (etas)
...
% Initialize the average steps vector
avgSteps_eta = zeros(1, length(eta_values));

% Run the trials for different eta values
for i = 1:length(eta_values)
    eta = eta_values(i);
    
    % Initialize the average steps for current eta
    ...
    % Calculate the average steps for current eta
    avgSteps_eta(i) = mean(avgStepsTrial);
end

% Plot the average steps for each eta
figure;
plot(eta_values, avgSteps_eta, '-o', 'LineWidth', 1.5);
title('Effect of Eta on Average Steps');
xlabel('Eta');
