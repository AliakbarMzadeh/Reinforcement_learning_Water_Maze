...
additionalReward = [8, 8];  % Additional reward location (green)
...
for trial = 1:numTrials
    ...
    while ~isequal(currentState, target) && ~isequal(currentState, cat)
        ...
        immediateReward = getImmediateReward(nextState, target, cat, additionalReward);
...
function immediateReward = getImmediateReward(nextState, target, cat, additionalReward)
    ...
    elseif isequal(nextState, additionalReward)
        immediateReward = 5;  % Additional reward stage
    else
        immediateReward = 0;
    end
end
