...
lambda = 0.5;  % Decay rate for eligibility trace
...
for trial = 1:numTrials
    ...
    eligibility = zeros(mapSize, mapSize);
    ...
    while ~isequal(currentState, target) && ~isequal(currentState, cat)
        ...
        TDerror = immediateReward + gamma * V(nextState(1), nextState(2)) - V(currentState(1), currentState(2));
        
        % Update the eligibility trace for the current state
        eligibility(currentState(1), currentState(2)) = 1;
        
        % Update the values of all visited states in the current trial
        V = V + eta * TDerror * eligibility;
...
% Plot the color map of the learned values
figure;
imagesc(V);
colorbar;
title('Learned Values');
xlabel('Column');
ylabel('Row');
